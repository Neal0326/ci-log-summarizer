import * as core from "@actions/core";

import {
  downloadJobLog,
  getWorkflowContext,
  listFailedJobs,
  type FailedJob,
  upsertPullRequestComment,
  writeJobSummary,
} from "./github";
import {
  normalizeConfidenceLevel,
  summarizeLogsWithOpenAI,
  type LogSummary,
} from "./openai";

const DEFAULT_MODEL = "gpt-4.1-mini";
const MAX_TOTAL_LOG_BYTES = 120_000;
const MAX_PER_JOB_LOG_BYTES = 60_000;
const LOG_SECTION_SEPARATOR = "\n\n==========\n\n";

type SummaryStatus = "success" | "fallback" | "skipped";
type OutputMode = "full" | "short";

interface PreparedJob extends FailedJob {
  log: string;
  originalBytes: number;
  sanitizedBytes: number;
  truncated: boolean;
}

function stripAnsi(text: string): string {
  return text.replace(
    // eslint-disable-next-line no-control-regex
    /\u001B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])/g,
    "",
  );
}

function escapeRegExp(value: string): string {
  return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

function gatherSecrets(explicitSecrets: string[]): string[] {
  const values = new Set<string>();

  for (const secret of explicitSecrets) {
    if (secret && secret.length >= 8) {
      values.add(secret);
    }
  }

  for (const [key, value] of Object.entries(process.env)) {
    if (!value || value.length < 8) {
      continue;
    }

    if (/(TOKEN|KEY|SECRET|PASSWORD|PAT)/i.test(key)) {
      values.add(value);
    }
  }

  return [...values];
}

function redactSecrets(text: string, secrets: string[]): string {
  let result = text;

  for (const secret of secrets) {
    result = result.replace(new RegExp(escapeRegExp(secret), "g"), "[REDACTED]");
  }

  return result
    .replace(/\b(?:gh[oprsu]_[A-Za-z0-9_]{16,}|github_pat_[A-Za-z0-9_]{20,})\b/g, "[REDACTED_GITHUB_TOKEN]")
    .replace(/\bsk-[A-Za-z0-9]{16,}\b/g, "[REDACTED_OPENAI_KEY]")
    .replace(/\b(?:AKIA|ASIA)[A-Z0-9]{16}\b/g, "[REDACTED_AWS_KEY]")
    .replace(/Bearer\s+[A-Za-z0-9._-]{10,}/gi, "Bearer [REDACTED]")
    .replace(/([?&](?:token|access_token|client_secret|signature|api_key)=)[^&\s]+/gi, "$1[REDACTED]")
    .replace(/((?:token|secret|password|api[_-]?key)\s*[:=]\s*["']?)[^\s"',]+/gi, "$1[REDACTED]");
}

function sliceUtf8Start(text: string, maxBytes: number): string {
  if (Buffer.byteLength(text, "utf8") <= maxBytes) {
    return text;
  }

  let low = 0;
  let high = text.length;

  while (low < high) {
    const mid = Math.ceil((low + high) / 2);
    const candidate = text.slice(0, mid);

    if (Buffer.byteLength(candidate, "utf8") <= maxBytes) {
      low = mid;
    } else {
      high = mid - 1;
    }
  }

  return text.slice(0, low);
}

function sliceUtf8End(text: string, maxBytes: number): string {
  if (Buffer.byteLength(text, "utf8") <= maxBytes) {
    return text;
  }

  let low = 0;
  let high = text.length;

  while (low < high) {
    const mid = Math.ceil((low + high) / 2);
    const candidate = text.slice(text.length - mid);

    if (Buffer.byteLength(candidate, "utf8") <= maxBytes) {
      low = mid;
    } else {
      high = mid - 1;
    }
  }

  return text.slice(text.length - low);
}

function sanitizeAndTruncateLog(
  rawLog: string,
  secrets: string[],
  maxBytes: number,
): Pick<PreparedJob, "log" | "originalBytes" | "sanitizedBytes" | "truncated"> {
  const originalBytes = Buffer.byteLength(rawLog, "utf8");
  let sanitized = stripAnsi(rawLog)
    .replace(/\r\n/g, "\n")
    .replace(/\r/g, "\n")
    .replace(/\0/g, "")
    .replace(/[^\S\n]+\n/g, "\n")
    .trim();

  sanitized = redactSecrets(sanitized, secrets);

  if (sanitized.length === 0) {
    sanitized = "Log content was empty after sanitization.";
  }

  const sanitizedBytes = Buffer.byteLength(sanitized, "utf8");

  if (sanitizedBytes <= maxBytes) {
    return {
      log: sanitized,
      originalBytes,
      sanitizedBytes,
      truncated: false,
    };
  }

  const marker = "\n\n[... log truncated to fit prompt budget ...]\n\n";
  const markerBytes = Buffer.byteLength(marker, "utf8");
  const headBudget = Math.min(18_000, Math.floor(maxBytes * 0.2));
  const tailBudget = Math.max(maxBytes - headBudget - markerBytes, 0);
  const truncated =
    `${sliceUtf8Start(sanitized, headBudget)}${marker}${sliceUtf8End(
      sanitized,
      tailBudget,
    )}`.trim();

  return {
    log: truncated,
    originalBytes,
    sanitizedBytes,
    truncated: true,
  };
}

function prepareJobs(
  jobs: Array<FailedJob & { rawLog: string }>,
  secrets: string[],
): PreparedJob[] {
  const perJobBudget = Math.max(
    16_000,
    Math.min(MAX_PER_JOB_LOG_BYTES, Math.floor(MAX_TOTAL_LOG_BYTES / Math.max(jobs.length, 1))),
  );

  return jobs.map((job) => {
    const processed = sanitizeAndTruncateLog(job.rawLog, secrets, perJobBudget);
    return {
      ...job,
      ...processed,
    };
  });
}

function extractKeyErrorLines(log: string): string[] {
  const patterns = [
    /\berror\b/i,
    /\bexception\b/i,
    /\bfailed\b/i,
    /\bfatal\b/i,
    /\bcannot\b/i,
    /\bunable to\b/i,
    /\bundefined reference\b/i,
    /\bmodule not found\b/i,
    /\bcommand not found\b/i,
    /\btraceback\b/i,
  ];

  const lines = log
    .split("\n")
    .map((line) => line.trim())
    .filter((line) => line.length > 0 && line.length <= 240);

  const unique = new Set<string>();
  const matches: string[] = [];

  for (let index = lines.length - 1; index >= 0; index -= 1) {
    const line = lines[index];

    if (
      line.startsWith("##[debug]") ||
      line.startsWith("Post job cleanup.") ||
      line.startsWith("Cleaning up orphan processes")
    ) {
      continue;
    }

    if (!patterns.some((pattern) => pattern.test(line))) {
      continue;
    }

    if (unique.has(line)) {
      continue;
    }

    unique.add(line);
    matches.push(line);

    if (matches.length >= 4) {
      break;
    }
  }

  return matches.reverse();
}

function collectKeyErrors(jobs: PreparedJob[]): string[] {
  const collected: string[] = [];
  const seen = new Set<string>();

  for (const job of jobs) {
    for (const line of extractKeyErrorLines(job.log)) {
      if (seen.has(line)) {
        continue;
      }

      seen.add(line);
      collected.push(line);

      if (collected.length >= 5) {
        return collected;
      }
    }
  }

  return collected;
}

function renderPrompt(preparedJobs: PreparedJob[], runUrl: string, workflow: string): string {
  const jobBlocks = preparedJobs.map((job) => {
    const failingSteps =
      job.failingSteps.length > 0
        ? job.failingSteps.join(", ")
        : "No failing step metadata available";
    const logNotes = [
      `original_bytes=${job.originalBytes}`,
      `sanitized_bytes=${job.sanitizedBytes}`,
      `truncated=${job.truncated ? "yes" : "no"}`,
    ].join(", ");

    return [
      `Job: ${job.name}`,
      `Conclusion: ${job.conclusion}`,
      `Status: ${job.status ?? "unknown"}`,
      `Started At: ${job.startedAt ?? "unknown"}`,
      `Completed At: ${job.completedAt ?? "unknown"}`,
      `Failing Steps: ${failingSteps}`,
      `Log Metadata: ${logNotes}`,
      "Logs:",
      job.log,
    ].join("\n");
  });

  const prompt = [
    "You are a senior DevOps engineer.",
    "Analyze the following GitHub Actions failure logs.",
    "",
    "IMPORTANT RULES:",
    "- Focus only on the FIRST real failure",
    "- Ignore cascading errors",
    "- Be concise and practical",
    "- Do NOT explain obvious things",
    "- Output MUST be valid JSON only",
    "",
    "Return JSON with:",
    "- shortSummary (1 short paragraph)",
    "- keyErrors (max 5 lines)",
    "- failedStep",
    "- likelyCause",
    "- suggestedNextSteps (actionable)",
    "- confidenceLevel (low|medium|high)",
    "",
    `Workflow: ${workflow}`,
    `Run URL: ${runUrl}`,
    "",
    jobBlocks.join(LOG_SECTION_SEPARATOR),
  ].join("\n");

  return sliceUtf8Start(prompt, MAX_TOTAL_LOG_BYTES + 10_000);
}

function normalizeMode(value: string): OutputMode {
  return value.trim().toLowerCase() === "short" ? "short" : "full";
}

function compactText(value: string | undefined, maxLength: number): string {
  const normalized = (value ?? "N/A").replace(/\s+/g, " ").trim();

  if (normalized.length <= maxLength) {
    return normalized;
  }

  return `${normalized.slice(0, Math.max(0, maxLength - 3)).trimEnd()}...`;
}

function buildDeterministicSummary(
  jobs: PreparedJob[],
  reason: string,
  status: "fallback" | "skipped",
): LogSummary {
  const primaryJob = jobs[0];
  const failedStep = primaryJob?.failingSteps[0] ?? "Unknown failing step";
  const keyErrors = collectKeyErrors(jobs);
  const firstError = keyErrors[0];
  const subject = primaryJob?.name ?? "the failed workflow job";
  const lead =
    status === "fallback"
      ? "OpenAI summarization failed, so this report was generated from the local workflow metadata and sanitized logs."
      : "AI summarization was skipped, so this report was generated from the local workflow metadata and sanitized logs.";

  return {
    shortSummary: `${lead} ${jobs.length} failed job(s) were detected, and the first actionable failure appears in ${subject} during ${failedStep}.`,
    keyErrors:
      keyErrors.length > 0
        ? keyErrors
        : ["No clear error line could be extracted from the sanitized log excerpt."],
    failedStep,
    likelyCause: firstError
      ? `The strongest local signal points to ${subject}, where the logs surface this likely primary failure: ${firstError}`
      : `The strongest local signal points to ${subject} around ${failedStep}, but the available log excerpt does not contain a single definitive error line.`,
    suggestedNextSteps: [
      `Open the workflow run and inspect ${subject} first.`,
      `Review the output around ${failedStep}.`,
      "Compare recent changes to workflow YAML, dependencies, environment variables, and test setup.",
      `If the issue is still unclear, re-run CI with more debug logging. Context: ${reason}`,
    ],
    confidenceLevel: normalizeConfidenceLevel(status === "fallback" ? "medium" : "low"),
  };
}

function renderMarkdown(
  status: SummaryStatus,
  summary: LogSummary,
  jobs: PreparedJob[],
  runUrl: string,
  workflow: string,
  model: string,
  mode: OutputMode,
  note?: string,
): string {
  if (mode === "short") {
    const firstJob = jobs[0];
    const shortSummary = compactText(summary.shortSummary, 220);
    const step = compactText(summary.failedStep || "unknown step", 80);
    const error = compactText(summary.keyErrors[0] ?? "N/A", 180);
    const jobName = compactText(firstJob?.name ?? "unknown", 60);

    return `<!-- ci-log-summarizer -->
### CI Summary

${shortSummary}

**Job:** ${jobName} | **Step:** ${step}
**Error:** ${error}
`;
  }

  const jobLines = jobs
    .map((job) => {
      const detail = job.failingSteps.length > 0 ? ` (${job.failingSteps.join("; ")})` : "";
      return `- \`${job.name}\`${detail}`;
    })
    .join("\n");
  const keyErrors = summary.keyErrors.map((line) => `- ${line}`).join("\n");
  const nextSteps = summary.suggestedNextSteps
    .map((step, index) => `${index + 1}. ${step}`)
    .join("\n");
  const renderedNote = note ? `${note}\n\n` : "";
  const statusDescriptions: Record<SummaryStatus, string> = {
    success: "OpenAI summarization completed successfully.",
    fallback: "OpenAI summarization failed, so a deterministic fallback summary was used.",
    skipped: "AI summarization was skipped.",
  };

  return `<!-- ci-log-summarizer -->
## CI Log Summarizer

**Workflow:** ${workflow}  
**Run:** [View workflow run](${runUrl})  
**Model:** \`${model}\`  
**Summary Status:** \`${status}\`

${statusDescriptions[status]}

${renderedNote}### Failed Jobs
${jobLines}

### Short Summary
${summary.shortSummary}

### Key Errors
${keyErrors}

### Failed Step
${summary.failedStep}

### Likely Cause
${summary.likelyCause}

### Suggested Next Steps
${nextSteps}

### Confidence Level
\`${summary.confidenceLevel}\`

Logs are sanitized, secret-redacted, and may be truncated before analysis.
`;
}

function renderNoFailuresMarkdown(runUrl: string, workflow: string): string {
  return `## CI Log Summarizer

**Workflow:** ${workflow}  
**Run:** [View workflow run](${runUrl})  
**Summary Status:** \`skipped\`

No completed failed jobs were found in the current workflow run attempt. Run this action from a dedicated follow-up job with \`if: failure()\` after the main CI jobs finish.
`;
}

function renderRetrievalFailureMarkdown(message: string): string {
  return `## CI Log Summarizer

**Summary Status:** \`skipped\`

The action could not retrieve enough workflow failure data from GitHub to build a summary.

${message}
`;
}

function buildLogDownloadPlaceholder(job: FailedJob, error: unknown): string {
  const message = error instanceof Error ? error.message : String(error);
  return [
    `Unable to download logs for job "${job.name}".`,
    `GitHub API error: ${message}`,
    "Proceed using workflow metadata only for this job.",
  ].join("\n");
}

function getSkipReason(
  openaiApiKey: string,
  eventName: string,
  isForkPullRequest: boolean,
): string | undefined {
  if (openaiApiKey.trim().length > 0) {
    return undefined;
  }

  if (isForkPullRequest && eventName === "pull_request") {
    return "This pull request originated from a fork and the OpenAI API key was not available. GitHub does not expose repository secrets to forked pull_request workflows, so AI analysis was skipped.";
  }

  return "The OpenAI API key input was empty, so AI analysis was skipped.";
}

async function finalizeOutputs(
  summaryStatus: SummaryStatus,
  failedJobCount: number,
  pullRequestCommented: boolean,
): Promise<void> {
  core.setOutput("summary-status", summaryStatus);
  core.setOutput("failed-job-count", String(failedJobCount));
  core.setOutput("pull-request-commented", String(pullRequestCommented));
}

async function run(): Promise<void> {
  let summaryStatus: SummaryStatus = "skipped";
  let failedJobCount = 0;
  let pullRequestCommented = false;

  try {
    const githubToken = core.getInput("github-token", { required: true });
    const openaiApiKey = core.getInput("openai-api-key").trim();
    const model = core.getInput("model") || DEFAULT_MODEL;
    const mode = normalizeMode(core.getInput("mode") || "full");
    const workflowContext = getWorkflowContext();

    core.info(
      `Inspecting workflow run ${workflowContext.runId} attempt ${workflowContext.runAttempt} in ${workflowContext.owner}/${workflowContext.repo}.`,
    );

    let failedJobs: FailedJob[];
    try {
      failedJobs = await listFailedJobs(githubToken, workflowContext);
    } catch (error) {
      const message = `Failed to retrieve workflow jobs from GitHub. ${
        error instanceof Error ? error.message : String(error)
      }`;
      await writeJobSummary(renderRetrievalFailureMarkdown(message));
      await finalizeOutputs(summaryStatus, failedJobCount, pullRequestCommented);
      core.setFailed(message);
      return;
    }

    failedJobCount = failedJobs.length;

    if (failedJobs.length === 0) {
      await writeJobSummary(
        renderNoFailuresMarkdown(workflowContext.runUrl, workflowContext.workflow),
      );
      await finalizeOutputs(summaryStatus, failedJobCount, pullRequestCommented);
      return;
    }

    core.info(`Found ${failedJobs.length} failed job(s). Downloading logs.`);

    const jobsWithLogs = await Promise.all(
      failedJobs.map(async (job) => {
        try {
          const rawLog = await downloadJobLog(githubToken, workflowContext, job.id);
          return {
            ...job,
            rawLog,
          };
        } catch (error) {
          core.warning(
            `Could not download logs for ${job.name}. Proceeding with workflow metadata only. ${
              error instanceof Error ? error.message : String(error)
            }`,
          );
          return {
            ...job,
            rawLog: buildLogDownloadPlaceholder(job, error),
          };
        }
      }),
    );

    const secrets = gatherSecrets([githubToken, openaiApiKey]);
    const preparedJobs = prepareJobs(jobsWithLogs, secrets);
    const skipReason = getSkipReason(
      openaiApiKey,
      workflowContext.eventName,
      workflowContext.isForkPullRequest,
    );
    const shouldSkipPullRequestComment =
      workflowContext.isForkPullRequest && workflowContext.eventName === "pull_request";
    let note: string | undefined;
    let summary: LogSummary;

    if (skipReason) {
      summaryStatus = "skipped";
      note = skipReason;
      summary = buildDeterministicSummary(preparedJobs, skipReason, "skipped");
      core.info(skipReason);
    } else {
      const prompt = renderPrompt(
        preparedJobs,
        workflowContext.runUrl,
        workflowContext.workflow,
      );

      try {
        core.info(`Sending ${preparedJobs.length} failed job log(s) to OpenAI model ${model}.`);
        summary = await summarizeLogsWithOpenAI({
          apiKey: openaiApiKey,
          model,
          prompt,
        });
        summaryStatus = "success";
      } catch (error) {
        const reason = error instanceof Error ? error.message : String(error);
        summary = buildDeterministicSummary(preparedJobs, reason, "fallback");
        summaryStatus = "fallback";
        note = `OpenAI summarization failed and a fallback summary was generated instead. ${reason}`;
        core.warning(note);
      }
    }

    const markdown = renderMarkdown(
      summaryStatus,
      summary,
      preparedJobs,
      workflowContext.runUrl,
      workflowContext.workflow,
      model,
      mode,
      note,
    );

    await writeJobSummary(markdown);

    if (workflowContext.pullRequestNumber != null && !shouldSkipPullRequestComment) {
      try {
        await upsertPullRequestComment(
          githubToken,
          workflowContext,
          workflowContext.pullRequestNumber,
          markdown,
        );
        pullRequestCommented = true;
        core.info(`Posted summary to PR #${workflowContext.pullRequestNumber}.`);
      } catch (error) {
        core.warning(
          `Failed to post the PR comment. The summary is still available in the job summary. ${
            error instanceof Error ? error.message : String(error)
          }`,
        );
      }
    } else if (workflowContext.pullRequestNumber != null) {
      core.info("Fork pull request detected. Skipping PR comment and leaving the result in the job summary.");
    } else {
      core.info("No pull request context detected. Wrote the summary to the job summary.");
    }

    await finalizeOutputs(summaryStatus, failedJobCount, pullRequestCommented);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);

    try {
      await writeJobSummary(renderRetrievalFailureMarkdown(message));
    } catch {
      // Ignore summary write failures here; the action still needs to report the main error.
    }

    await finalizeOutputs(summaryStatus, failedJobCount, pullRequestCommented);
    core.setFailed(message);
  }
}

void run();
